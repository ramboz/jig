...
- **Ground factual claims about runnable surfaces** (library/API capability,
  version/perf behavior, behavior of existing code) by an executed probe (run a
  command, read source/`node_modules`) or a citation. Everything you cannot
  verify goes in the ADR's `## Assumptions` section, explicitly marked — never
  asserted as fact. Populate `## Kill criteria` where a meaningful one exists.
  Both sections are risk-gated: write "None" when there's nothing to record
  rather than inventing boilerplate. (Spec 064-02 / ADR-0020.)
