...
- [x] `docs/specs/README.md` regenerated by `workflow.py status-board`.
      ...
      **Done — row populated with canonical supersede shape + refusal
      matrix + slice 005-01 dev-log #6 closure + two-phase-commit
      watch-item pointer.**
- [x] `CLAUDE.md` hygiene per spec 025-01: this slice closes spec
      ...
      **Done — skills-table row updated to list `supersede <old>
      <new>` with the canonical shape summary. Active-specs section
      already empty pre-slice (per the slice 025-01 hygiene baseline);
