---
status: DONE
dependencies: [028-01]
last_verified: 2026-06-03
---

## Slice 049-01 — claim-and-release-on-transition

**Goal:** Teach `workflow.py transition <slice> IN_PROGRESS` to stamp
a `claimed_by:` identifier in slice frontmatter and reserve the
transition on `origin/main` (with PR fallback). Two parallel
worktrees calling `transition <same-slice> IN_PROGRESS` simultaneously
get one success and one structured refusal naming the existing claim.

**DoR:**
- ✅ Spec 028-01's reserve-on-main + race-on-push + PR-fallback
  primitives are in production in `adr.py` and `workflow.py new`.
- ✅ Slice frontmatter parser (slice 015-01) supports adding/removing
  arbitrary keys.
- ✅ `_SubprocessRecorder` mock pattern (spec 003-03 / 028-01)
  available for tests.

**Acceptance Criteria:**

1. **`transition <slice> IN_PROGRESS` stamps `claimed_by:`.** The
   helper derives the identifier from `git rev-parse --abbrev-ref
   HEAD` (or `JIG_CLAIM_ID` env override if set) and writes it to
   the slice frontmatter alongside the `status:` flip.
2. **Reserve-on-main with PR fallback.** Commits the frontmatter
   change to `origin/main`. Push-refused-by-policy → PR fallback
   branch + `gh pr create`. Push-refused-by-race → instruct re-run.
   Mirrors spec 028-01 classifier verbatim.
3. **Collision refusal.** If the on-disk slice already has
   `claimed_by:` set to a different identifier AND `status: IN_PROGRESS`,
   the helper refuses with a structured error naming the existing
   `claimed_by:` and pointing to `--release` as the escape hatch.
4. **Claim cleared on forward transition.** `transition <slice>
   REVIEWED` clears `claimed_by:` from frontmatter. Same for any
   back-transition to `READY_FOR_IMPLEMENTATION` / `DRAFT`.
5. **`--release` flag.** `workflow.py transition <slice>
   READY_FOR_IMPLEMENTATION --release --reason "<text>"` clears
   `claimed_by:` and resets `status:`. Refuses without `--reason`.
   The reason is appended to a `## Release log` section in the slice
   for audit.
6. **`--no-push` parity.** Local-only claim path, same shape as
   `workflow.py new --no-push`. Default refuses if origin is
   unreachable; `--no-push` opts into local stamping with a warning.
7. **Subprocess-mocked tests.** No real `git push` exercised in the
   suite. Use `_SubprocessRecorder` to assert the helper invokes
   `git fetch origin main`, computes the next state, commits, and
   pushes in the expected order.
8. **Live-remote dogfood.** Documented in the deviation log:
   transition this slice itself to `IN_PROGRESS` and verify the
   commit lands on `origin/main` and a parallel worktree's
   `transition` call sees the existing claim.

**DoD:**
- [x] All ACs pass; full test suite green (no regressions). — 2085 tests, OK.
- [x] Implementer test coverage exercises each AC with at least one
      fixture. Edge cases: collision refusal, `--release` without
      `--reason`, `--push` (opt-in; see deviation 1), race-on-push recovery,
      claim cleared on REVIEWED, claim cleared on back-transition.
- [x] Reviewed by `reviewer` subagent (compliance pass). — verdict pass,
      `reviews/slice-01-compliance.md`.
- [x] Craft pass via `pr-review` (no `[blocker]`-tagged findings). — verdict
      pass, `reviews/slice-01-craft.md`.
- [x] Implementation review passed.
- [x] Deviation log produced under this slice heading.
- [x] Reconciliation review passed.
- [x] `docs/refinement-todo.md` updated if any decisions were
      deferred during implementation. — no new undefined cases; the two
      craft nits are tracked in the deviation log below (§5).

### Close-out (post-DONE)

- [x] `docs/specs/README.md` regenerated by `workflow.py status-board`.
      Notes column receives a one-liner pinning the `claimed_by:`
      contract surface.
- [x] `CLAUDE.md` updates: spec 049 added to Active specs (one slice
      down, one to go); compress will fire when 049-02 lands.
- [x] Inline-mirror block in `workflow.py` bracketed with
      `# ---------- Slice 049-01: claim-on-transition ----------`
      markers naming the helpers mirrored from spec 028-01. Per
      ADR-0002's three-callers rule, extraction defers until a third
      caller of `reserve-on-main` emerges.

**Anti-horizontal-phasing check:** End-to-end value: a user running
two worktrees that each call `transition <same-slice> IN_PROGRESS`
simultaneously gets exactly one success and one structured refusal
naming the existing claim — observable from CLI output and exit code.
The slice ships a working refusal; the next slice surfaces the claim
in the status board.

### Deviation log (after reconciliation)

The original spec is preserved above. Implementation notes:

1. **Claims are LOCAL by default; `--push` / `--pr` opt into the
   origin/main reservation (deviation from AC2/AC6 wording).** The slice
   text framed the IN_PROGRESS claim as reserve-on-origin *by default*
   with `--no-push` as the opt-out (mirroring `workflow.py new`). On
   explicit user direction the default was inverted: `transition …
   IN_PROGRESS` stamps `claimed_by:` locally with **no network**, and
   `--push` (direct) / `--pr` (via PR) opt into reserving the claim on
   origin/main. Rationale: keep the everyday "start a slice" path
   offline-friendly and non-blocking; the spec's *intent* (a structured
   refusal when a parallel worktree already holds the claim) is preserved
   on the opt-in path plus a local on-disk collision check (AC3). AC6's
   "default refuses if origin unreachable" is reinterpreted as "refuses
   when `--push` is chosen and origin is unreachable"
   (`test_push_unreachable_origin_refuses`). The `--no-push` flag named in
   the AC is therefore absent — subsumed by making local the default.

2. **A single uniform detached-worktree reservation serves every
   branch (incl. `main`).** `workflow.py new` routes on-branch (in-place
   on `main`, detached worktree off-`main`) because it *creates* files.
   The claim *edits an existing file*, and a detached checkout of
   `origin/main` is collision-free from any branch — so the reservation
   uses one detached-worktree path everywhere, reading the origin/main
   copy via `git show origin/main:<path>` for the collision check and
   pushing the claim commit BY SHA from `project_dir` (the 051 lesson).
   Simpler than duplicating an on-`main` in-place variant; fully mockable.

3. **Claims are a frontmatter-layout feature.** Stamping `claimed_by:`
   into a legacy *prose-only* slice (no frontmatter) would synthesize a
   spurious `---` block between the heading and the `**STATUS:**` line, so
   claim bookkeeping is a no-op there and `--push`/`--pr`/`--release` on
   such a slice refuse with a clear "requires a file-per-slice layout"
   message.

4. **Reviewer finding folded back in (was needs-changes → pass).** The
   first compliance pass caught a real bug: the PR-fallback branch was
   `claim/{raw slice label}`, but the human label carries spaces / an
   em-dash and is an invalid git ref — the push would fail on a real
   remote (the mocks hid it). Fixed by slugging the label through a new
   `_ref_safe()` helper, and both PR-fallback tests now assert the pushed
   `refs/heads/claim/…` ref is whitespace-free / a valid git ref
   (`_assert_claim_branch_ref_valid`). Re-review verdict: pass.

5. **Craft nits — deferred (no `refinement-todo` entry; no new undefined
   case).** (a) `_append_release_log` appends the entry to the end of the
   slice section rather than anchored under the `## Release log` heading —
   correct for the current single-trailing-section shape; revisit if a
   release log ever grows or gains following prose. (b) The "foreign claim
   still IN_PROGRESS" predicate appears twice (on-disk check in
   `transition()` and origin/main check in `_reserve_claim_on_main`) —
   intentionally separate sources; per ADR-0002 a *third* caller would
   trigger extracting a shared `_is_foreign_active_claim()` predicate.

6. **AC8 live-remote dogfood — done.** *Local leg:* `transition 049-01
   IN_PROGRESS` stamped `claimed_by: claude/competent-banach-f69ece`, and
   the REVIEWED transition cleared it (AC4), both observed on disk.
   *Live-remote leg (with go-ahead):* the `--push` reserve path was run
   against the real `origin/main` — leg 1 landed the claim commit
   `a98372a` (`origin/main`'s 049-01 → `status: IN_PROGRESS`,
   `claimed_by: claude/competent-banach-f69ece`); leg 2, a parallel
   session (`JIG_CLAIM_ID=wt-parallel-session`), fetched the updated
   `origin/main`, saw the claim, and **refused** with the structured
   error naming the holder + the `--release` escape hatch. The dogfood
   claim was then released on `origin/main` (commit `7b5e6f5` restores the
   slice to its pre-claim DRAFT state) so the trunk does not misleadingly
   show 049-01 IN_PROGRESS while the implementation is unmerged. (The
   live legs were driven through the exact `--push` helper
   `_reserve_claim_on_main` rather than a `transition … IN_PROGRESS
   --push` CLI call, to avoid regressing this slice's own RECONCILED
   status on the branch — same code path, no branch-state side effect.)
   Minor follow-up noted: the collision error interpolates the spaced
   slice label into the suggested `--release` command, so the hint isn't
   copy-pasteable — cosmetic, parked.

7. **Docs updated:** `skills/spec-workflow/SKILL.md` (transition flags +
   claim contract), `docs/workflow.md` (claim-on-IN_PROGRESS note), the
   `spec-workflow` row in `CLAUDE.md`, and the status-board Notes column.

8. **New shared helper:** `_common/parsing.clear_frontmatter_field()`
   (idempotent field removal) with its own unit tests — the natural
   counterpart to `set_frontmatter_field`, used to clear `claimed_by:`.
