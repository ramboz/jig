...
- [x] `docs/refinement-todo.md` updated if any decisions were
      deferred during implementation. (ADR-0037 cross-reference corrected;
      no new deferral introduced.)
...
- [x] `docs/specs/README.md` regenerated by `workflow.py status-board`.
      ...
      (it's preserved across regen). — Done; the marker-keyed exemption,
      the two gate points, `JIG_BUG_CLOSURE_GATE`, and the effort-and-protocol
      floor are written into the 091-01 Notes cell.
- [x] Primer hygiene per spec 025-01 rule: **if this slice closes the
      ...
      one-liner only for cross-cutting facts. — Done; both primers
      compressed in lockstep, invariants migrated to the board Notes,
      glossary term **Bug-fix repository closure** added. No new skill.
...
| `docs/memory/**` | `deferred (memory-sync)` | Any learning worth capturing (the frame-critique reshape, the prose-gate-vs-bug-005 tension) is written by `/jig:memory-sync` at session close, alongside the glossary entry — not hand-edited here. `learnings.md` gets no inline edit from this slice. |
| `docs/workflow.md` (Key document — bug lifecycle gates) | `updated` | The "Three gates give the lifecycle its teeth" enumeration was factually wrong after this slice added two; corrected inline to five, naming the inventory + call-site closure gates and `JIG_BUG_CLOSURE_GATE` (ADR-0010 live-prose norm). `templates/docs/workflow.md.template` has no equivalent enumeration, so the fix is scoped to the repo doc. |
| `skills/bug-fix/SKILL.md` prose (beyond the deliverable edits) | `updated` | Two surfaces in the same file that this slice's feature edits made stale: the teeth-gates table gained the closure rows (a deliverable edit), and the "The three distinctive gates" summary paragraph — which enumerated exactly three — was corrected to name the repository-closure gates as a fourth distinctive category. Host packages regenerated for the change. |
...
| [ADR-0016](../../decisions/adr-0016-bug-fix-lifecycle.md) (bug-fix lifecycle record) | `no-op` (deliberate) | ADR-0016's "three distinctive gates" prose + teeth-gate table now omit the two closure gates, but it is a **decision record**: per [ADR-0010](../../decisions/adr-0010-amendment-scope-records-vs-live-prose.md) a record is not rewritten inline for a later decision change — the change is carried by a new ADR (ADR-0037, which declares `dependencies: [… adr-0016 …]`). Left uncorrected by design; noted here so the omission reads as intentional, not missed. |
