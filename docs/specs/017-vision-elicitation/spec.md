...
4. **Structural-match dogfood (worked example #1):** the worked
   ...
   whose H2 section structure matches `docs/product-vision.md`
   byte-for-byte in section headings. Body content is the user's
   words, not asserted byte-for-byte.
5. **Structural-match dogfood (worked example #2):** the worked
   example transcript for the YarnFinder pitch produces an output
   document whose section structure matches what
   `/Users/ramboz/Projects/CLAUDE.md` prescribes (Vision statement,
   Target users, Core problem, Competitive landscape summary,
   prioritized backlog, MVP scope, Future scope, Data sourcing,
...
**Definition of Done:**
- All ACs 1–8 green pre-merge; AC #9 deferred to post-merge close-out.
- New surface tests green; no regression in existing suite.
- Implementation review passes (reviewer subagent).
- Reconciliation review passes after deviation log lands.
- Spec status board updated.
- Close-out (post-DONE): routing-dogfood verification + CLAUDE.md
  hot-cache row for the new skill.
