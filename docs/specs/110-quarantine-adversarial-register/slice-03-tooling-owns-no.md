...
|----------|-------------|-----------|
| `docs/specs/README.md` | `updated` | Regenerated by `workflow.py status-board` (110-03 status). |
| `skills/independent-review/SKILL.md` | `no-op` (verified) | A2 classification found refusals already tool-attributed; no rewrite warranted (deviation #1–3). |
| `skills/spec-workflow/SKILL.md` | `no-op` (verified) | Same — refusal narration already tool-owned; body unchanged. |
| `skills/bug-fix/SKILL.md` | `no-op` (verified) | Same — refusal narration already tool-owned; body unchanged. |
| `hosts/**` mirrors | `no-op` | No SKILL body changed, so no mirror rebuild for this slice (drift stays green). |
| `scripts/test_working_posture.py` | `updated` | New `ToolOwnedRefusalTests` guard (tool-owned refusals + invoke imperative). |
