...
| `skills/{independent-review,spec-workflow,bug-fix}/SKILL.md` | `updated` | Posture pointer + ADR-0056 link at body top (AC3). |
...
| `docs/decisions/README.md` / ADR index | `updated` | ADR-0056 accepted + indexed (`README.md:61`). |
| `docs/architecture.md` | `no-op` | No module-boundary / public-contract change — prose posture only. |
| `docs/memory/**` (for 110-01) | `no-op` | 110-01 adds no domain term beyond ADR-0056 (indexed). NB: `learnings.md` was modified on this branch by the co-landed **bug 033** fix (its "Bug 033" entry), not 110-01 — accounted for under bug 033's own reconciliation, not an omission here. |
