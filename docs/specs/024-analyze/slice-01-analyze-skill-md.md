...
- [x] `docs/specs/README.md` regenerated by `workflow.py status-board`.
      _(Notes curated to match 012-01 / 022-01 shape; references the
      bundled constitution-gate piece.)_
- [x] `CLAUDE.md` updates: hot-cache entry for spec 024-01; Skills
      table row added; sprint-focus refresh.
      _(Added 024-analyze Active-specs entry; new `/jig:analyze`
      Skills-table row; sprint-focus updated alongside spec 023's
      close-out to acknowledge the spec-kit gap closure.)_
- [x] `docs/product-vision.md` "Tier 1 — default-on" list updated
      to include `analyze` (now eight entries — also reflects
      `clarify` from spec 023 and `contracts` from spec 022).
      Principle 5 updated alongside spec 023's close-out to
      acknowledge that the deferral-hint pattern is per-skill,
      not universal.
