...
- [x] `docs/specs/README.md` regenerated by `workflow.py status-board`.
      ...
      governs closed-spec drift; drift #5 → 038." (036-02's Notes
      column now carries the full sweep summary + drift #5 → 038
      deferral; 036-01's row already cites ADR-0008 with Option C
      + scope.)
- [x] `CLAUDE.md` hygiene per spec 025-01 rule: this slice closes
      ...
      **Compress** spec 036's Active-specs entry — drop facts
      ...
      shipped). (No compression needed: Active-specs section was
      already `_(none — …)_` at slice start; the Closed-spec drift
      policy term landed in the Key terms section during 036-01's
      reconciliation, naming ADR-0008 as the canonical rule. Status
      board Notes column carries the per-slice invariants. No
      Skills-table edit needed.)
