...
- [ ] Dogfood: run `migrate.py copy-machinery` against a real
      ...
      greenfield. Log result in the deviation log. _(Implementer ran
      an in-suite end-to-end dogfood against a tmpdir spec-driven
      fixture — 10 jig-* skills + 3 agents + 5 hook scripts + valid
      settings.json marker on every entry — but the real-project
      dogfood against shallow-validator is still pending.)_
