...
| [037-git-origin-safety](037-git-origin-safety/spec.md) | 037-02 — reserve-against-origin | READY_FOR_IMPLEMENTATION | External-review brief 03. workflow.py bugs #3 + #4: `_next_spec_number` reads `origin/main` via `git ls-tree` in push mode; preflight refuses on diverged local main (shared exit code). Race classifier retained as last-resort catch. May close refinement-todo "race-on-disk". |
