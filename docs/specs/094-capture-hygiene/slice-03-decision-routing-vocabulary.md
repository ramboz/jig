...
11. **Re-measured at RECONCILED, because no independent measurement existed.**
    All three reviewers of this slice ran without Bash and said so, so every
    routing figure in this log traced back to its own author — a gap the
    reconciliation review named explicitly and asked to close before DONE. Re-run
    on the final tree:

    ```
    $ python3 scripts/skill_routing.py --min-rank1 0.85
    positive triggers: 62/62 routed within top_k  (rank-1 rate 59/62 = 95%)
    negative triggers: 44/44 routed away correctly
    ratchet: rank-1 rate 95% vs floor 85% — PASS
         0.20  adr-workflow  ×  memory-sync
      thresholds: ~ warn ≥0.50   !! error ≥0.75   (0 hazard(s))
      [PASS] +  rank 1/1  «Remember this decision so we don't rewrite it tomorrow»
      [PASS] +  rank 2/3  «Record this decision: we shortened the cards, …»

    $ python3 scripts/run_tests.py
    Ran 3339 tests in 89.213s
    OK (skipped=4)

    $ python3 scripts/build_host_packages.py --check
    OK: committed host packages are in sync with source.

    $ uvx --from ruff==0.15.16 ruff check .      # the pinned CI version
    All checks passed!
    ```

    Every figure in §1–§5 holds. (`scripts/ci_check.py` itself could not run
    locally — it shells out to `pipx`, which is absent on this machine; the pinned
    ruff was run directly through `uvx` instead, and CI runs the real gate.)
...
| Primer surfaces: `CLAUDE.md` / `AGENTS.md` / scaffold templates | `no-op` | Per EngTip #23 / spec 076 the primer deliberately does *not* re-list skill descriptions — the host surfaces them each session — so a description edit has no primer surface to update. No Active-specs entry owed either (see 094-01's sweep). |
