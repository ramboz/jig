...
## Managed-file metadata must derive from source copy rules
Slice 033-04 initially built runtime managed-file metadata by scanning target directories after install. Craft review caught that this could accidentally mark pre-existing user-owned .claude/skills, agents, or hook helper files as jig-managed. The safe pattern is to derive metadata specs from the same plugin source allow-lists used by the copy/install path, then hash only files that actually materialized at the expected destination.
