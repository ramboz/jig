...
## Bug-fix repository closure

[ADR-0037](../decisions/adr-0037-bug-fix-repository-closure-evidence.md) / [spec 091](../specs/091-bug-fix-repository-closure/spec.md) — two teeth gates on the bug lifecycle so a narrow patch cannot duplicate existing logic or leave a convergent path unfixed (the Mystique PR-3417 failure mode). `bug.py new` stamps a **creation-time `closure_schema:` frontmatter marker** and emits `## Repository closure inventory` (equivalent/convergent logic, history, affected call sites, reuse decision) + `## Call-site closure` (post-fix disposition). New standard/gnarly records gate `ROOT_CAUSED→FIXING` on the inventory and `→REVIEWED` on the call-site closure; `JIG_BUG_CLOSURE_GATE=0` (deliberateness, not human-only). **Marker-keyed, never section-keyed** (the load-bearing compatibility choice — [spec 091 frame-critique](../specs/091-bug-fix-repository-closure/reviews/slice-01-frame-critique.md)): a legacy pre-091 record has no marker and is exempt; a marked record that deletes the sections still gates, so *evasion-by-omission* ≠ *legacy-by-omission*. The parser is a **deliberateness/effort-and-protocol floor**, not a completeness proof ([ADR-0011](../decisions/adr-0011-spec-gate-model.md) lineage): it rejects a bare negative verdict ("none found") but accepts an honest "not closable by name search — assumption" *with* a recorded protocol; the discovery-quality judgment lives in the `bug-review` pass, which applies the [ADR-0052](../decisions/adr-0052-grounding-enumeration-for-universal-claims.md) enumeration standard to negative claims. Tool-neutral (semantic index preferred; targeted search + `git log`/`git blame` floor). Falsifiability = paired leading kill indicators (vacuity + effect, sampled from records via the marker) alongside the lagging primary-miss criterion. trivial-tier records are exempt (deleted at triage). ADR accepted after four frame-critique passes.
