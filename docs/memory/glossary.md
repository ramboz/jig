...
Shipped decisions settled mid-session that fall outside spec slices, recorded in docs/decisions/lightweight-decisions.md. Routing heuristic: lightweight iff (a) no module-boundary/contract/policy change AND (b) a future agent would need it to avoid undoing it; else ADR; refinement-todo if still open; drop if ephemeral. Template: Date, Decision, Context, Scope, Commit (optional). **Phase 1 (083-01..03, DONE, ADOPTED 2026-06-25):** the browsable home + scaffold seeding + reconcile/memory-sync *nudges*. **Phase 2 (083-04..07, DRAFT — food-log report 2026-06-25):** the real failure mode is recall-dependence (decisions lost regardless of size because nothing *remembers* to ask), so a deterministic session-end **scan** in a Stop hook (sibling of jig-task-capture.sh — zero orchestrator-token cost) replaces the nudge, plus a routing rubric + decisions.py add-lightweight helper (083-05) and a companion widening of the reconciliation ADR trigger to also fire on "load-bearing design choice with rejected alternatives" (083-06, own ADR). Design constraints: owner-gated writes, mandatory provenance (who + quote), precision over recall.
