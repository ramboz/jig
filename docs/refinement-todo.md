...
**Resolved by [Spec 033](specs/033-host-adapter-portability/spec.md).** The direct Codex-support ask fired the original trigger. [Slice 033-02](specs/033-host-adapter-portability/slice-02-agents-md-canonical-primer.md) made `AGENTS.md` the canonical scaffolded primer while `CLAUDE.md` became the Claude adapter; [slice 033-05](specs/033-host-adapter-portability/slice-05-codex-scaffold-adapter.md) implemented Codex scaffold mode; and [slice 033-06](specs/033-host-adapter-portability/slice-06-codex-plugin-packaging.md) implemented Codex plugin packaging. The remaining Codex role-agent gap is tracked below as the TOML adapter / plugin-discovery decision.
...
**Deferred:** Slice 033-06 packages the canonical `agents/*.md` prompts in the Codex plugin output as non-discoverable source. Current Codex plugin docs define plugin discovery for `.codex-plugin/plugin.json`, `skills/`, and `hooks/hooks.json`; current Codex custom-agent docs define discoverable agents as TOML files under project-local or user-local `.codex/agents/`. Adding an unsupported `agents` field to `.codex-plugin/plugin.json`, or claiming Markdown prompts are discoverable Codex agents, would create a false contract.
**Resolution trigger:** Codex documents plugin-level custom-agent discovery, a Codex user asks for jig roles as real Codex custom agents, or a future Codex scaffold/plugin slice claims custom-agent discovery parity.
**Mitigation idea:** Render `.codex/agents/*.toml` from the canonical `agents/*.md` prompts, preserving the capability-honesty notes; if Codex exposes a plugin agent manifest/path later, add that adapter layer without changing the source prompts.
