...
### Decision: extend the scaffold-precondition gate to `adr.py new`
**Deferred:** [Spec 063](specs/063-scaffold-precondition-gate/spec.md) (DONE) hardened the **spec-creation** door (`workflow.py new` classify-and-route + the `spec-workflow` SKILL.md Step 0 precondition) so an unscaffolded project is routed to `/jig:scaffold-init` / `/jig:migrate` instead of dead-ending or improvising structure. The **ADR-creation** door (`adr.py new` + the `adr-workflow` skill) has the *same shape* and would benefit from the same treatment, but spec 063 was scoped to spec creation **per direction** (see its non-goals: "Gating `adr.py new` … Noted as a future follow-on, not built"). The primitives are now in place — `skills/_common/scaffold_state.py` `classify_scaffold_state()` is reusable as-is — so this is wiring + a SKILL.md Step 0 mirror, not new detection.
**Resolution trigger:** A user reports the same failure on ADR creation (an unscaffolded project gets a dead-end refusal, or an auto-triggered `adr-workflow` run improvises a `docs/decisions/` skeleton), OR `adr.py new` / the `adr-workflow` SKILL.md is touched for another reason. Mirror spec 063: consume `classify_scaffold_state` in `adr.py`'s reserve path with the same `JIG_SCAFFOLD_PRECONDITION` bypass, and add the parallel Step 0 prose to `adr-workflow` SKILL.md.
