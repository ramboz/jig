...
flowchart TB
    ...
    subgraph hookspine["Deterministic spine — 11 hooks"]
        direction TB
...
- **Hook spine** intercepts at five Claude Code event types (SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop) via eleven hook scripts. Two are async log-only — never block, never inject (`telemetry`, `skill-trace`); seven can inject `additionalContext` (`context-check`, `memory-scan`, `post-edit-verify`, `boundary-change-warn`, `task-capture`, `decision-capture`, `jig-semantic-index`); two can block tool calls with exit-code 2 (`spec-gate`, `secret-scan`). The two Stop hooks (`task-capture`, `decision-capture`) are siblings — same scan-and-surface pattern, one for tasks, one for decisions (spec 083-04).
...
- `scripts/` — top-level repo tooling, not skill helpers: `usage.py` (per-spec token/cost reporting, spec 056), `verify_install.py`, `spec_lint.py`, `validate_manifests.py`, `build_release_zip.py`, `build_codex_plugin.py`, the `*_contract.py` builders, and `run_tests.py`
