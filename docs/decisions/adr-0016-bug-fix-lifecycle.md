...
- **RESOLVED_ON_MAIN** — terminal off-ramp after root cause: fresh
  `origin/main` no longer reproduces the bug, so another parallel session
  already landed the fix and this session should not create a duplicate patch.
...
| `ROOT_CAUSED → FIXING` | fresh-main recheck recorded as still reproducing, `fix_class` declared, and the `regression_test` runs **red** | `bug.py main-check` stamps `main_repro_*`; `transition` shells to `tdd.py` (targeted), expects exit 1; stamps `red_confirmed_at` |
...
**The three distinctive gates** are the diagnose gate (`→ ROOT_CAUSED`,
the ≥2-hypotheses anti-anchoring rule), the **fresh-main recheck** after
root cause, and the **red→green teeth**: the helper itself *witnesses*
the regression test fail before the fix and pass after, so "there is a
regression test" is machine-attested, not a writer's claim. A bug that
is already fixed on fresh main becomes `RESOLVED_ON_MAIN`; a test that is
already green without the fix does not capture the bug, and the
`→ FIXING` gate refuses it.
...
gate), `JIG_BUG_MAIN_CHECK_GATE=0` (fresh-main recheck), and
`JIG_BUG_TEST_GATE=0` (red→green teeth) — separate vars so one can be
relaxed without the others. A `tdd.py` env error (exit 2) **fails
closed** (gate not satisfied), distinct from red.
...
main_repro_checked_at:    # stamped by bug.py main-check after ROOT_CAUSED
main_repro_ref:           # origin/main@<sha> checked before FIXING
main_repro_result:        # reproduces | resolved_on_main
