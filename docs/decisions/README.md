...
- [ADR-0041: scaffold-init defaults to plugin mode](adr-0041-scaffold-defaults-to-plugin-mode.md) — reverses spec 016-03's in-repo default: a no-flag scaffold produces a lean plugin-mode project; copying jig's machinery in-repo becomes the explicit `--in-repo` opt-in for CI, cloud agents, or plugin-less teammates. (2026-07-29, Accepted)
