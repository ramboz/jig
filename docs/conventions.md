...
**Rule:** Standalone investigations — open exploration *before* a decision is named — live as **research notes** at `docs/research/R-NNN-<slug>.md`, not crammed into the inbox and not forced into an ADR or spec ([ADR-0054](decisions/adr-0054-research-notes-artifact-convention.md), spec 108).
**Why:** A research note is the *open investigation phase* — sources, pros/cons, open questions, no decision named yet. It is **sequential with, not a competitor to**, `refinement-todo.md`: refinement-todo holds a *named deferred decision + resolution trigger*, and a research note *promotes into* one (or an ADR / a spec) once it crystallizes, citing `R-NNN` in the downstream artifact's Context. Filing open research directly as a refinement-todo entry would force a fake trigger and pollute that trigger-audit register; leaving it inline in the inbox makes the inbox a deep-research dump instead of a thin capture layer. Numbering is **local-and-cheap** — an `R-NNN` number is *not* reserved on origin/main like specs/ADRs, so a collision is a harmless nuisance reconciled by hand. The `docs/research/00`–`09` files are jig's **frozen seed research** (its founding bootstrap corpus), not living notes. There is deliberately no `research.py` helper, index-regen, or link-linter yet (deferred in `refinement-todo.md` pending real triggers).
**How to apply:** Copy `docs/research/TEMPLATE.md` to `docs/research/R-NNN-<slug>.md` (next free number), set `status: OPEN`, and add a row to the hand-maintained index `docs/research/README.md`. When an inbox item grows thick, move the depth into a note and leave a one-line pointer (`[date] exploring X → R-NNN`). On crystallization, write the ADR / spec / refinement-todo entry citing `R-NNN`, then flip the note's `status` to `CONCLUDED` (or `ABANDONED`) and add a `Promoted to:` line.
