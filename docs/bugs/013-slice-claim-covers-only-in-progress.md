...
- [~] H1: The claim field is simply under-scoped — it should be stamped in
  ...
  *causal* claim: the narrow scope is stated design intent (`IN_PROGRESS` =
  ...
  decision, recorded in [ADR-0045](../decisions/adr-0045-slice-claim-covers-active-lifecycle.md)
...
recorded as [ADR-0045](../decisions/adr-0045-slice-claim-covers-active-lifecycle.md)
+ a spec 049 amendment. Options 2 (a separate expiring lease field) and 4 (a
touched-on-another-branch advisory) remain unbuilt and are logged as ADR-0045
...
recorded as a decision ([ADR-0045](../decisions/adr-0045-slice-claim-covers-active-lifecycle.md))
...
1. **A claim is still LOCAL by default.** Across two worktrees on unpushed
   ...
   does not close it; see [ADR-0045](../decisions/adr-0045-slice-claim-covers-active-lifecycle.md)
...
3. **The trunk claim is advisory outside `IN_PROGRESS`, deliberately.** Both
   ...
   **unclaimed** `IN_PROGRESS` trunk copy is equally off-limits, while our **own**
   ...
   Logged as ADR-0045 open question 4.
4. **`READY_FOR_REVIEW` is classified as a working state on judgment, not
   ...
   transitions into it and runs the review in the same session. ADR-0045 kill
...
7. **The warning has few opportunities to fire until claims are pushed by
   ...
   without `--push`. ADR-0045 open question 6 states this rather than counting
...
2. **Widened the trunk reservation call site but not the callee.**
   ...
   would have manufactured precisely the false-refusal class ADR-0045 claims to
...
5. **Widened a hard refusal on the trunk path without noticing.** Same shape as
   ...
   refused outright, verbatim the false-block class ADR-0045 claims to avoid. It
...
6. **Recorded two regression tests that were never written.** Entry 5's fix was
   ...
   **both** paths. Dropping `target_is_in_progress` from the trunk refusal, or
   ...
   green — so the false-refusal case ADR-0045 names as the thing it avoids had no
...
- `test_local_foreign_in_progress_claim_does_not_block_a_working_target` /
  ...
  anti-regression on the relaxation. These pin the case ADR-0045 names as the
...
Each carries an in-place `AMENDED by ADR-0045` note. Note the count *fell* from
...
[ADR-0045](../decisions/adr-0045-slice-claim-covers-active-lifecycle.md) OQ1,
