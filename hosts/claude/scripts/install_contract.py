...
# ---------------------------------------------------------------------------
#
# This module is the single source of truth for the release/package file set:
# the EXCLUDE side above (`is_excluded_release_path`) and the INCLUDE side
# here. The **shipping consumers** of the INCLUDE constants are the committed
# host-package builders — `build_claude_plugin.py` (whole
...
# contract in one place (exercised by the install-contract tests); it is kept
# as the executable definition of the contract rather than as a live builder
# dependency.
...
# test_build_claude_plugin.py::RuntimeScriptsShippedTests and
# test_build_release_zip.py::ClaudeZipShapeTests.test_scripts_are_the_runtime_allowlist_only.
...
# The subset of the runtime-scripts allowlist appropriate for the **Codex**
# package. `spec_lint.py` is host-neutral (pure stdlib, no `.claude/` or
# `${CLAUDE_PLUGIN_ROOT}` assumptions) and the rendered Codex `migrate` /
# `analyze` skills invoke it as `${PLUGIN_ROOT}/scripts/spec_lint.py`, so it
# must ship. The rest of the allowlist — the `verify_install` /
