...
> `## Clarifications` section if any, append new entries) Codex can
...
target directory already contains a `.codex/` folder with no
...
- **Verbatim-answer rule.** The skill does not paraphrase, expand, or
  ...
  reject the operation when a .codex/ directory is present in the
