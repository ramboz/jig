...
- **Any other user-installed PR/code-review skill.** Codex user location:
  `$HOME/.agents/skills/pr-review/` — but the deferral is **category-based,
  ...
  **defer to it.** Codex can select the
  more specific skill by description; if you want to be sure, explicitly
  invoke that skill. The one exception jig's description carves out is the
...
1. **Full repo context (preferred).** You're inside a Codex session
...
- **The deferral hint is the routing mechanism, not a code path.** Jig's
  ...
  review surface area will win. If Codex consistently picks jig's
  baseline over such a skill, jig's description is too greedy — open an
  issue. **This richer-skill deferral applies to _interactive_ use only.**
  The spec-workflow **craft pass** spawns a read-only `reviewer` subagent
  with no `Skill` tool, so it cannot use interactive skill selection — there
...
- **Lightweight is a feature, not a limitation.** The baseline does not
  ...
  at the user scope (commonly `$HOME/.agents/skills/pr-review/`).
