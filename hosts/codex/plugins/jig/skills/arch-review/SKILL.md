...
> the scope, classify the domain) Codex can run inline. If any other
...
> review, Codex can prefer that one over jig's
...
install a heavier skill at the user scope and Codex can prefer it.
...
- **Any other user-installed architecture/design/RFC-review skill.**
  Codex user location: `$HOME/.agents/skills/arch-review/` — but the deferral
  ...
  explicitly invoke that skill. The one exception jig's description carves
...
- **The deferral hint is the routing mechanism, not a code path.**
  Jig's description tells Codex "prefer any other
  ...
  surface area will win. If Codex consistently picks jig's
  baseline over such a skill, jig's description is too greedy — open
  an issue. **This richer-skill deferral applies to _interactive_ use
  only.** The spec-workflow **arch pass** spawns a read-only `reviewer`
  subagent with no `Skill` tool, so it cannot use interactive skill selection —
  there
  `review.py` uses a host-specific file-read fallback outside this Codex-rendered skill-selection path. Codex-rendered guidance keeps richer-skill deferral to interactive skill selection or explicit invocation. See
...
- **Lightweight is a feature, not a limitation.** The baseline does
  ...
  the user scope (commonly `$HOME/.agents/skills/arch-review/`).
