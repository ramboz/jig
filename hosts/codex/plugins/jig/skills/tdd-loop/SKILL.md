...
```bash
python3 "${PLUGIN_ROOT}/skills/tdd-loop/tdd.py" detect [target]
```
...
```bash
python3 "${PLUGIN_ROOT}/skills/tdd-loop/tdd.py" run [target] [--test-path PATH]
python3 "${PLUGIN_ROOT}/skills/tdd-loop/tdd.py" run [target] [--test SELECTOR]
```
...
This skill is the *tooling layer* that implementer subagents (and Codex in
