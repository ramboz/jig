...
> Codex owns the judgment (what the decision actually says).
...
The script does file mutation deterministically. Codex is responsible for
...
```bash
python3 "${PLUGIN_ROOT}/skills/adr-workflow/adr.py" new <slug> \
...
feature branch or a linked `.codex/worktrees/*` worktree — it reserves
...
Then Codex fills in Context / Options Considered / Recommended Decision /
...
```bash
python3 "${PLUGIN_ROOT}/skills/adr-workflow/adr.py" accept <NNNN>
```
...
```bash
python3 "${PLUGIN_ROOT}/skills/adr-workflow/adr.py" \
...
```bash
python3 "${PLUGIN_ROOT}/skills/adr-workflow/adr.py" index docs/decisions
```
...
straight into the README is silently overwritten on the next regen. If a row
reads badly, fix the ADR's opening paragraph — that is the summary's source.
...
Read-only; exits non-zero on either problem it can find. **Stale index** — an
ADR changed and `index` wasn't re-run (or a summary was hand-edited).
**Duplicate ADR number** — two ADRs claim one number, which is what parallel
branches produce when the number was never reserved on the trunk. The index
renders both bullets without complaint and a staleness check can't see it (both
*are* faithfully derived), so it needs its own detector; the message names both
files so you know which to renumber.
...
# 3. Codex edits the file: fills Context, Options, Recommended, Consequences.
...
- **The helper does NOT spawn a Task or commit anything.** It only mutates
  files. Codex is responsible for orchestration (e.g. running
