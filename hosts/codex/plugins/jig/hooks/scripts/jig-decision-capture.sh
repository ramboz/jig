...
# runs as a plugin (${CODEX_HOME}/hooks/scripts/) or a scaffolded install
# (${CODEX_PROJECT_DIR}/.codex/hooks/scripts/). Fully fail-open: any error
...
try:
    ...
    messages = messages_from_payload(
        data, os.environ.get('CODEX_PROJECT_DIR', '.'))
