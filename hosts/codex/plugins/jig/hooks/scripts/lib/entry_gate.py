...
resolved against the docs base) or under ``.jig`` / ``.codex`` / ``.git``.
...
# Both host adapter dirs are listed so a Codex session treats an also-present
# `.codex/` as infra (and vice versa) — dual-host robustness. NOTE the Codex
# build rewrites `.codex`→`.codex` here (blind global transform), so the Codex
# copy becomes (".jig", ".codex", ".codex", ".git"): harmless dup, but it means
# a Codex session still nudges on a `.codex/` edit. That residual dual-host
...
_INFRA_DIRS = (".jig", ".codex", ".codex", ".git")
