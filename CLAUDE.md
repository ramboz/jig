...
- **Slice-claim on IN_PROGRESS** — [spec 049](docs/specs/049-slice-claim-on-in-progress/spec.md) (049-01 reconciled — DONE pending the AC8 live-remote `--push` dogfood go-ahead; 049-02 status-board-claim-rendering DRAFT) — extends the spec 028 reserve-on-main pattern to **slice ownership**: `workflow.py transition <slice> IN_PROGRESS` stamps `claimed_by:` (current branch name, or `JIG_CLAIM_ID`) on a frontmatter slice and refuses a foreign claim that's still IN_PROGRESS (naming the holder + pointing at `--release`). **Deviation from AC2/AC6 (user-directed):** the claim is **local by default** (offline-friendly "start a slice"); `--push` / `--pr` opt into reserving it on `origin/main` via a uniform ephemeral detached-worktree (collision-checked against `git show origin/main:<path>`, pushed BY SHA; race/protected-branch handling mirrors `workflow.py new`). Claim cleared on REVIEWED / back-transition; `--release --reason` force-clears + logs to a `## Release log`. New shared helper `_common/parsing.clear_frontmatter_field`. Inline-mirrors 028-01/003-03 (ADR-0002 second caller; extract on a third). 049-02 will surface the claim in the status board.
