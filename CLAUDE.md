...
- **Session git-freshness** — [ADR-0048](docs/decisions/adr-0048-session-git-freshness-fetch-and-nudge.md) / [spec 103](docs/specs/103-session-git-freshness/spec.md) — fail-open `SessionStart` nudge fired at *time-zero*: fetches (timeout-guarded, best-effort) the branch's integration base and warns when `HEAD` is behind, before a stale premise forms. **Smart-target resolution** — a non-own `@{upstream}` (git-flow/fork base) else `origin/main`→`origin/master`; the own-remote guard keeps a pushed task branch measuring *base drift* not own-branch advancement (the load-bearing frame-critique fix). Complements bug 001's command-time warning; always-fetch over the proposal's fetch-free bias (timeout is the never-hang safety); `JIG_GIT_FRESHNESS=0`.
