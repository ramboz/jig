...
- **Branch routing** — `main` is the sole active line: 1.x and the 2.0 multi-host work both shipped here (latest tag `v2.13.0`). The legacy `v2` branch is closed — don't route work to it. See [docs/roadmap.md](docs/roadmap.md).
- **Active specs:** work has shipped through spec 113 — the [status board](docs/specs/README.md) is the live source, always check it before picking up work. A few parked drafts remain (034 federation-tier, 062 refactor-workflow, 090 immutable-release-bytes, 105 durable-failure-quarantine); 114/115 are recorded-not-built (ADR-0062/0063 Proposed, from [R-001](docs/research/R-001-cloudflare-adlc-assessment.md)); 083 lightweight-decision-records is IN_PROGRESS (083-08 Codex host-validation handoff open). Recent load-bearing closures are indexed under Key terms (leanness lens 109, first-class blockers 111, adversarial-register quarantine 110, ref-aware lifecycle + claim reservation 112, tri-host Copilot 113 — GitHub Copilot CLI is jig's 3rd committed host, plugin-only, full **package** parity; residuals in [refinement-todo](docs/refinement-todo.md)). **Bug-fix repository closure** — [ADR-0037](docs/decisions/adr-0037-bug-fix-repository-closure-evidence.md) (Accepted) / [spec 091](docs/specs/091-bug-fix-repository-closure/spec.md) — two marker-keyed closure gates on the bug lifecycle; `JIG_BUG_CLOSURE_GATE=0`. Per-slice invariants live in the status-board Notes column. **Long-horizon-autonomy bridge (oh-my-cli follow-on):** spec 106 — autonomy governance plane & identity separation ([ADR-0051](docs/decisions/adr-0051-autonomy-governance-plane.md), Accepted) is **built** (see the Governance-plane term below); spec 105 — durable failure-quarantine jig-half ([ADR-0050](docs/decisions/adr-0050-durable-failure-quarantine.md), Proposed) is still **recorded, not built**. servo halves live in `ramboz/servo` (servo specs 023/024/025, ADR-0029/0030) — not yet built. See [docs/specs/README.md](docs/specs/README.md). Deferred → [docs/refinement-todo.md](docs/refinement-todo.md).
...
- **Security floor** — [ADR-0013](docs/decisions/adr-0013-security-floor-policy.md) / [spec 052](docs/specs/052-security-scaffold/spec.md) — 5-part scaffolded floor; defense-in-depth, not a firewall. Sibling **Governance plane** — [ADR-0051](docs/decisions/adr-0051-autonomy-governance-plane.md) / [spec 106](docs/specs/106-autonomy-governance-plane/spec.md) — `scaffold-init` scaffolds CODEOWNERS + protected-path CI + `<docs>/governance.md` and mirrors `governance.PROTECTED_PATHS` (self-referential) into `scaffold.json.protected_paths` (`jig-boundary-change-warn` soft-nudges in-boundary); **inert until branch protection is armed** (files, not server-side settings); autonomy precondition = merge-*capability* separation (not name), fail-safe when unknown, via `governance.py identity-check` (servo 023's signal).
...
- **Context-cost discipline** / **Thin-orchestrator** — [spec 055](docs/specs/055-context-cost-discipline/spec.md) + [057](docs/specs/057-thin-orchestrator/spec.md) — cost ≈ orchestrator context × turns (delegate reads, keep the primer lean); turn count + peak context are the top knobs; `workflow.py session-plan` dispatches.
...
- **Slice-claim on working states** — [spec 049](docs/specs/049-slice-claim-on-in-progress/spec.md) / [ADR-0045](docs/decisions/adr-0045-slice-claim-covers-active-lifecycle.md) — `transition` stamps `claimed_by:` on WORKING states, releases on the pickup queue + terminal; blank ≠ free. Sibling **Ref-aware lifecycle checks + claim reservation** — [ADR-0058](docs/decisions/adr-0058-cross-ref-lifecycle-state-check.md) / [spec 112](docs/specs/112-cross-ref-lifecycle-state/spec.md) — lifecycle checks consult sibling/remote refs, not just the checkout (the duplicate-work incident). Four classes: **A** already-integrated on `origin/main` → hard gate (`land.py` backstop + `transition` guard, keyed on branch-*introduced* ADRs not deps); **B** concurrent both-ends-`IN_PROGRESS` → extends ADR-0045 cross-ref via a `refs/claims/<N>` CAS reservation (identity read = hard block; CAS advisory; liveness = manual `--release`; fires on default **and** `--push`/`--pr`); **C** sequential re-do of work DONE-on-a-sibling (the *reported incident*) → sibling *evidence-complete* `DONE` read → halt-and-reconcile; **D** uncommitted/offline → advisory (DEFERRED). Shared `_common/cross_ref_state.py` + `claim_ref.py`; `JIG_CROSSREF_GATE=0` / `--reopen` bypass. **DEFERRED (refinement-todo):** guard-family unification (trigger fired 112-05), Class-D advisory (112-06), landed-at anchor (112-07).
...
- **Recent review/posture closures** — **Leanness-review lens** ([ADR-0055](docs/decisions/adr-0055-leanness-lens-folds-into-existing-passes.md) / [spec 109](docs/specs/109-leanness-review-lens/spec.md)): over-engineering / YAGNI check folded into the **arch + reconciliation passes**, no new gate — *don't re-propose a standalone leanness gate*. **First-class blockers** ([ADR-0057](docs/decisions/adr-0057-first-class-blockers-are-annotations.md) / [spec 111](docs/specs/111-first-class-blockers/spec.md)): `blocked_by:` annotation on an *actionable* slice (not a lifecycle state) → board `## Blocked slices`; distinct from `DEFERRED`/`dependencies:`. **Adversarial-register quarantine** ([ADR-0056](docs/decisions/adr-0056-adversarial-register-quarantine.md) / [spec 110](docs/specs/110-quarantine-adversarial-register/spec.md)): the flaw-hunting register is *bounded to the review passes* (isolated reviewer subagents); outside a review, default collaborative and solution-forward.
...
Host packaging details live in [docs/architecture.md](docs/architecture.md); Codex plugin packaging is built by `scripts/build_codex_plugin.py` (with `--install-codex-agents` for the explicit custom-agent TOML install step), and Copilot plugin packaging by `scripts/build_copilot_plugin.py` — all three hosts regenerated + drift-guarded by the single `scripts/build_host_packages.py`.
