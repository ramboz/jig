...
- ADR-0004 (decisions-folder-naming) **accepted** — jig's default ADR layout will rename `docs/adrs/` → `docs/decisions/` with `adr-NNNN-` filename prefix. Implementation is deferred to a future slice (likely folded into 008-03 jig-self-migration); existing `docs/adrs/` layout in jig is untouched for now. Two follow-up open questions parked in refinement-todo (own spec vs. fold-in, backwards-compat window for `adr.py`).
- 008-migrate-existing-project: **first Tier 0 sibling spec to scaffold-init**; slice 008-01 (migrate-report) **DONE** — `migrate.py report <dir>` produces a read-only 5-section migration report (Inventory / Mapping / Conflicts / Ambiguities / Operations) with verdict `adoptable | partial | not-yet-spec-driven`. 34 tests; dogfooded against aso-shallow-validator (verdict: adoptable, 32 slices / 23 ADRs / 4 spikes / 5 milestones inventoried). Two review passes; two regression tests added for reviewer-flagged latent bugs (README leak in slices/spikes, line-ref drift in deviation log). Slices 008-02 (rename-decisions), 008-03 (jig-self-migration), 008-04 (slice-to-spec-mapping), 008-05 (scaffold-init --migrate suggestion) deferred.
...
| `/jig:migrate` | Slice 008-01 DONE — active, auto-triggering; `migrate.py report <dir>` produces a read-only 5-section migration report for projects with existing spec-driven structure. Sibling to scaffold-init (Tier 0). Future subcommands `rename-decisions` (008-02), `slice-to-spec` (008-04) deferred. | Yes (auto + explicit) |
