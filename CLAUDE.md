...
- 013-release-pipeline: **second dev-infrastructure spec** (no new skill, no Tier; same shape as 011); four slices authored 2026-05-14, all `READY_FOR_IMPLEMENTATION`. **013-01 ci-baseline** (`.github/workflows/ci.yml` + `pr-title.yml` + `scripts/validate_manifests.py`), **013-02 release-please-scaffold** (release-please action + config + manifest + `plugin.json` bump 0.1.0 → 1.0.0 + seed `CHANGELOG.md`), **013-03 release-zip-artifact** (`build_release_zip.py` + `package` job + smoke test), **013-04 marketplace-rename-and-docs** (rename `jig-dev` → `jig` + README install rewrite + MIT LICENSE + CONTRIBUTING update + dogfood). User-locked decisions: release-please tool, marketplace rename to `jig`, first release `v1.0.0`, MIT license. Approved plan at `~/.claude/plans/i-think-it-s-time-flickering-quilt.md`.
