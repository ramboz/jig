...
class GreenfieldScaffoldTests(unittest.TestCase):
    ...
    def test_draft_markers(self):
        ...
        # AGENTS.md + CLAUDE.md + every .md under docs/
        md_files = [self.target / "AGENTS.md", self.target / "CLAUDE.md"] + sorted(
    ...
    # AC #6: AGENTS.md from template + Hot Cache + project name substituted
    def test_agents_md_hot_cache(self):
        agents_md = (self.target / "AGENTS.md").read_text()
        self.assertIn("Hot Cache", agents_md)
        self.assertIn("demo-project", agents_md, "project name not substituted")
        # No raw template placeholders left
        self.assertNotIn("{{PROJECT_NAME}}", agents_md)

    def test_claude_md_is_adapter_to_agents_md(self):
        claude_md = (self.target / "CLAUDE.md").read_text()
        self.assertIn("@AGENTS.md", claude_md)
        self.assertIn("Claude Code adapter", claude_md)
        self.assertNotIn("## Hot Cache", claude_md)
...
class DocContentTests(unittest.TestCase):
    ...
    # AC #7 (001-02): AGENTS.md Hot Cache codenames includes project name
    def test_agents_md_codename_includes_project_name(self):
...
class SignalDetectionTests(unittest.TestCase):
    ...
    def test_llm_agent_signals_record_offer(self):
        ...
        (self.target / ".github" / "copilot-instructions.md").write_text("# Copilot\n")
        self._scaffold()
        signals = self._manifest()["scaffold_signals"]
    ...
    def test_jig_managed_agents_md_does_not_self_trigger_llm_signal(self):
        (self.target / "AGENTS.md").write_text(
            "# Prior partial scaffold\n\n"
            "> Generated by [jig](https://github.com/ramboz/jig)\n"
        )
        self._scaffold()
...
class WizardQATests(unittest.TestCase):
    ...
    def test_user_owned_agents_md_refuses_without_force(self):
    ...
    def test_agents_md_watermark_prose_refuses_without_force(self):
        (self.target / "AGENTS.md").write_text(
            "# Existing project instructions\n\n"
            "This project discusses the phrase Generated by [jig] in prose.\n"
        )
        result = run_scaffold(self.target)
        self.assertEqual(result.returncode, 3)
        self.assertIn("AGENTS.md", result.stderr)
        self.assertIn("Generated by [jig] in prose", (self.target / "AGENTS.md").read_text())

    def test_user_owned_claude_md_refuses_without_force(self):
        (self.target / "CLAUDE.md").write_text("# Existing Claude instructions\n")
        result = run_scaffold(self.target)
        self.assertEqual(result.returncode, 3)
    ...
    def test_force_overwrites_user_owned_agents_md(self):
        (self.target / "AGENTS.md").write_text("# Existing project instructions\n")
        env = os.environ.copy()
        env["CLAUDE_PLUGIN_ROOT"] = str(REPO_ROOT)
        result = subprocess.run(
        ...
        self.assertEqual(result.returncode, 0, f"stderr: {result.stderr}")
        self.assertIn("Generated by [jig]", (self.target / "AGENTS.md").read_text())
...
class VisionTemplateSlotsTests(unittest.TestCase):
    ...
    # AC #5: AGENTS.md.template's "What this project does" line points to vision doc
    def test_agents_md_template_references_product_vision(self):
        body = self._read("templates/AGENTS.md.template")
        ...
        self.assertIsNotNone(
            what_section,
            "AGENTS.md.template must have a '## What this project does' section",
        )
    ...
    def test_claude_md_template_is_adapter_to_agents_md(self):
        body = self._read("templates/CLAUDE.md.template")
        self.assertIn("@AGENTS.md", body)
        self.assertNotIn(
            "## Hot Cache", body,
            "CLAUDE.md.template should not duplicate the canonical hot cache",
        )
