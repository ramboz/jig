...
class WithMachineryDocsRegressionTests(unittest.TestCase):
    ...
    passed: docs/, primer files, scaffold.json, etc. still appear."""
    ...
    def test_canonical_files_still_created(self):
        for rel in ("AGENTS.md", "CLAUDE.md", "scaffold.json",
...
class HostRendererBoundaryTests(unittest.TestCase):
    """Slice 033-03 — Claude scaffold output now flows through an explicit
    host renderer boundary, without emitting future Codex files."""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="jig-033-03-renderer-")
        self.target = Path(self.tmpdir) / "demo-project"
        self.target.mkdir()
    ...
    def test_renderer_abstraction_is_explicit(self):
        self.assertTrue(hasattr(scaffold_mod, "HostRenderer"))
        self.assertTrue(hasattr(scaffold_mod, "ClaudeScaffoldRenderer"))
        self.assertTrue(
            issubclass(
                scaffold_mod.ClaudeScaffoldRenderer,
                scaffold_mod.HostRenderer,
            )
        )
        with self.assertRaises(TypeError):
            scaffold_mod.HostRenderer(REPO_ROOT, self.target)

        renderer = scaffold_mod.ClaudeScaffoldRenderer(
            REPO_ROOT, self.target, force=False,
        )
        for method in (
            "render_primers",
            ...
            "install_agents",
            "install_hooks",
            "translate_hook_protocol",
            "bind_paths",
            "managed_file_metadata",
            "primer_managed_file_specs",
            "runtime_managed_file_specs",
            "verification_fixtures",
        ):
            self.assertTrue(callable(getattr(scaffold_mod.HostRenderer, method)))
            self.assertTrue(callable(getattr(renderer, method)))

    def test_claude_renderer_owns_path_rewrites(self):
        skill_body = (
            "python3 ${CLAUDE_PLUGIN_ROOT}/skills/spec-workflow/workflow.py"
        )
        self.assertEqual(
            scaffold_mod.ClaudeScaffoldRenderer.rewrite_skill_md_paths(skill_body),
        ...
        self.assertEqual(
            scaffold_mod.ClaudeScaffoldRenderer.rewrite_hook_command(command),
        ...
        self.assertEqual(
            scaffold_mod.ClaudeScaffoldRenderer(
                ...
                "block_reason": "Spec gate failed.",
            }),
            {
                "continue": True,
                "additionalContext": "Heads up.",
                "exit_code": 2,
                "stderr": "Spec gate failed.",
            },
        )

    def test_claude_scaffold_generated_tree_shape_pinned(self):
        r = run_scaffold_with_args(self.target)
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        ...
        skills = sorted(
            p.name for p in (self.target / ".claude" / "skills").iterdir()
            if p.is_dir()
        )
        ...
        agents = sorted(
            p.name for p in (self.target / ".claude" / "agents").glob("*.md")
        )
        ...
        hook_scripts = sorted(
            p.name
            for p in (self.target / ".claude" / "hooks" / "scripts").glob(
                "jig-*.sh"
            )
        )
        self.assertEqual(hook_scripts, [
            "jig-boundary-change-warn.sh",
            "jig-context-check.sh",
            "jig-memory-scan.sh",
            "jig-post-edit-verify.sh",
        ...
        self.assertEqual(sorted(settings["hooks"].keys()), [
            "PostToolUse",
            "PreToolUse",
            "SessionStart",
            "Stop",
    ...
    def test_plugin_only_preserves_no_copied_runtime_or_codex_files(self):
        r = run_scaffold_with_args(self.target, "--plugin-only")
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        for rel in (
            ".claude/skills",
            ...
            self.assertFalse(
                (self.target / rel).exists(),
                f"unexpected scaffolded runtime path: {rel}",
            )


class CodexScaffoldAdapterTests(unittest.TestCase):
    """Slice 033-05 — Codex scaffold output is generated through the host
    adapter boundary without requiring Claude scaffold files."""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="jig-033-05-codex-")
        self.target = Path(self.tmpdir) / "demo-project"
        self.target.mkdir()
    ...
    def test_codex_agents_materialize_with_capability_limitations(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        agents_dir = self.target / ".codex" / "agents"
        for name in ("architect", "implementer", "reviewer"):
            agent = agents_dir / f"jig-{name}.md"
            self.assertTrue(agent.is_file(), f"missing Codex agent: {agent}")
            text = agent.read_text()
            self.assertIn("Codex adapter note", text)
            self.assertIn("cannot enforce", text)
            self.assertNotIn(".claude", text)
            self.assertNotIn("CLAUDE.md", text)
            expected_body = scaffold_mod.CodexScaffoldRenderer.rewrite_agent_body(
    ...
    def test_codex_hooks_json_registers_all_logical_hooks(self):
        ...
        self.assertTrue(
            hooks_payload.get("metadata", {}).get("managed_by_jig")
        )
        hooks = hooks_payload["hooks"]
        self.assertEqual(sorted(hooks.keys()), sorted(EXPECTED_HOOK_EVENTS))
        ...
        scripts_dir = self.target / ".codex" / "hooks" / "scripts"
        for script in EXPECTED_HOOK_SCRIPTS:
            path = scripts_dir / script
            self.assertTrue(path.is_file(), f"missing hook script: {path}")
            self.assertEqual(path.stat().st_mode & 0o777, 0o755)

    def test_codex_hook_scripts_are_rewritten_for_codex_runtime(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        ...
        telemetry = (scripts_dir / "jig-telemetry.sh").read_text()
        self.assertIn(".codex", telemetry)
        self.assertIn("CODEX_PROJECT_DIR", telemetry)
    ...
    def test_codex_refuses_existing_unmanaged_hooks_before_runtime_copy(self):
        ...
        self.assertFalse(
            ...
            "Codex hook refusal must happen before runtime skills copy",
        )
        self.assertEqual(
            list((self.target / ".codex").glob("hooks/scripts/jig-*.sh")),
            [],
            "Codex hook refusal must not leave hook scripts behind",
        )

    def test_codex_runtime_templates_are_available_to_copied_helpers(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        for rel in (
            ".codex/templates/docs/decisions/adr-0000-template.md",
            ".codex/templates/docs/specs/slice-template.md",
            ".codex/hooks/hooks.json",
        ):
            self.assertTrue(
                (self.target / rel).is_file(),
                f"missing Codex runtime support file: {rel}",
            )

        helper = self.target / ".codex" / "skills" / "jig-adr-workflow" / "adr.py"
        r = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    ...
                    f"path = {str(helper)!r}; "
                    "spec = importlib.util.spec_from_file_location('adr', path); "
                    "mod = importlib.util.module_from_spec(spec); "
                    "spec.loader.exec_module(mod); "
                    "sys.exit(0 if mod._template_path().is_file() else 2)"
                ),
            ],
            cwd=self.target,
            capture_output=True,
    ...
    def test_codex_scaffolded_scaffold_helper_can_render_codex_again(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        nested = Path(self.tmpdir) / "nested-project"
        bad_claude_root = Path(self.tmpdir) / "bad-claude-plugin-root"
        bad_claude_root.mkdir()
        helper = (
            self.target / ".codex" / "skills" / "jig-scaffold-init"
            / "scaffold.py"
        )
        r = subprocess.run(
            [sys.executable, str(helper), "--host", "codex", str(nested)],
            cwd=self.target,
            capture_output=True,
            text=True,
            env={"CLAUDE_PLUGIN_ROOT": str(bad_claude_root)},
        )
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        self.assertTrue((nested / "AGENTS.md").is_file())
        self.assertTrue((nested / ".codex" / "hooks.json").is_file())
        self.assertTrue(
            (nested / ".codex" / "skills" / "jig-scaffold-init"
             / "SKILL.md").is_file()
        )
        self.assertFalse((nested / ".claude").exists())
        self.assertFalse((nested / "CLAUDE.md").exists())

    def test_codex_runtime_has_unprefixed_helper_aliases_for_peer_loads(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        alias = self.target / ".codex" / "skills" / "scaffold-init"
        self.assertTrue((alias / "scaffold.py").is_file())
        self.assertFalse(
            (alias / "SKILL.md").exists(),
            "compatibility aliases must not register duplicate Codex skills",
        )

        migrate = self.target / ".codex" / "skills" / "jig-migrate" / "migrate.py"
        r = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    "import importlib.util, sys; "
                    f"path = {str(migrate)!r}; "
                    "spec = importlib.util.spec_from_file_location('migrate', path); "
                    "mod = importlib.util.module_from_spec(spec); "
                    "sys.modules[spec.name] = mod; "
                    "spec.loader.exec_module(mod); "
                    "plugin = mod._resolve_plugin_root(); "
                    "loaded = mod._load_scaffold_module(plugin); "
                    "sys.exit(0 if hasattr(loaded, 'copy_machinery') else 2)"
                ),
            ],
            cwd=self.target,
            capture_output=True,
    ...
    def test_codex_migrate_skill_keeps_copy_machinery_claude_only(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        skill = self.target / ".codex" / "skills" / "jig-migrate" / "SKILL.md"
        text = skill.read_text()
        self.assertIn("Codex adapter note", text)
        self.assertIn("copy-machinery` remains Claude-only", text)
        self.assertIn(
            'python3 "${CODEX_PROJECT_DIR:-$PWD}/.codex/skills/'
            'jig-migrate/migrate.py" report',
            text,
        )

        section = text.rsplit("## Copying machinery into your project", 1)[1]
        section = section.split("## Agentic slice-to-spec migration", 1)[0]
        self.assertIn("${CLAUDE_PLUGIN_ROOT}/skills/migrate/migrate.py", section)
        self.assertIn(".claude/skills/", section)
        self.assertIn("${CLAUDE_PROJECT_DIR}/.claude", section)
        self.assertNotIn(".codex/skills/", section)
        self.assertNotIn("${CODEX_PROJECT_DIR:-$PWD}/.codex", section)
        self.assertIn(
            "Cross-reference rewrites in text files under `docs/`, "
            "`CLAUDE.md`",
            text,
        )
        self.assertIn("Claude-shaped scan scope", text)
        self.assertNotIn("only touches `docs/`, `AGENTS.md`, and `.codex/`",
                         text)

    def test_codex_scaffolded_migrate_helper_keeps_claude_shaped_scope(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        existing = Path(self.tmpdir) / "existing-project"
        files = {
            "docs/adrs/001-foo.md": "# ADR-0001 Foo\n",
            "docs/architecture.md": (
                "See docs/adrs/001-foo.md and 001-foo.md.\n"
            ),
            "CLAUDE.md": "Claude ref docs/adrs/001-foo.md.\n",
            ".claude/notes.md": "Claude runtime ref docs/adrs/001-foo.md.\n",
            "AGENTS.md": "Codex primer ref docs/adrs/001-foo.md.\n",
            ".codex/notes.md": "Codex runtime ref docs/adrs/001-foo.md.\n",
        }
        ...
        migrate = self.target / ".codex" / "skills" / "jig-migrate" / "migrate.py"
        r = subprocess.run(
            [sys.executable, str(migrate), "rename-decisions", str(existing)],
            cwd=self.target,
            capture_output=True,
            text=True,
            env={},
        ...
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        self.assertFalse((existing / "docs" / "adrs").exists())
        self.assertTrue(
            (existing / "docs" / "decisions" / "adr-0001-foo.md").is_file()
        )

        for rel in ("docs/architecture.md", "CLAUDE.md", ".claude/notes.md"):
            text = (existing / rel).read_text()
            self.assertIn("docs/decisions/adr-0001-foo.md", text)
            self.assertNotIn("docs/adrs/", text)

        self.assertIn("docs/adrs/001-foo.md",
                      (existing / "AGENTS.md").read_text())
        self.assertIn("docs/adrs/001-foo.md",
                      (existing / ".codex" / "notes.md").read_text())

    def test_codex_hook_protocol_fixtures_cover_warning_and_blocking(self):
        renderer = scaffold_mod.CodexScaffoldRenderer(REPO_ROOT, self.target)
        self.assertEqual(
            renderer.translate_hook_protocol({
                "continue": True,
                "additional_context": "Context is getting full.",
            }),
            {
                "continue": True,
                "additionalContext": "Context is getting full.",
            },
        )
        self.assertEqual(
            renderer.translate_hook_protocol({
                "block_reason": "docs/conventions.md requires approval.",
            }),
            {
                "exit_code": 2,
                "stderr": "docs/conventions.md requires approval.",
            },
        )

    def test_codex_generated_warning_hook_emits_codex_payload(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        script = self.target / ".codex" / "hooks" / "scripts" / "jig-context-check.sh"
        env = os.environ.copy()
        env.update({
            "CODEX_PROJECT_DIR": str(self.target),
            "JIG_CONTEXT_WINDOW_BYTES": "100",
            "JIG_CONTEXT_SOFT_WARN_PCT": "0.01",
        })
        r = subprocess.run(
            ["bash", str(script)],
            input=json.dumps({
        ...
        self.assertIn("Context-fill warning", payload["additionalContext"])

    def test_codex_generated_blocking_hook_exits_two(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        script = self.target / ".codex" / "hooks" / "scripts" / "jig-spec-gate.sh"
        payload = {
            "session_id": "codex-test",
        ...
        self.assertIn("Blocked: docs/conventions.md", r.stderr)

    def test_codex_managed_files_track_codex_renderer(self):
        r = self._run_codex_scaffold()
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        ...
        by_path = {
            entry["path"]: entry
            for entry in manifest["managed_files"]["files"]
        }
        for rel in (
            "AGENTS.md",
            ".codex/skills/jig-scaffold-init/SKILL.md",
            ".codex/skills/scaffold-init/scaffold.py",
            ".codex/templates/docs/decisions/adr-0000-template.md",
            ".codex/agents/jig-reviewer.md",
            ".codex/hooks/scripts/jig-context-check.sh",
            ".codex/hooks/hooks.json",
            ...
            self.assertEqual(by_path[rel]["host_renderer"], "codex")

        self.assertNotIn("CLAUDE.md", by_path)
        self.assertFalse(any(path.startswith(".claude/") for path in by_path))

        markers = {
            marker["path"]: marker["marker"]
            for marker in manifest["managed_files"]["policy"]["inline_markers"]
        }
        self.assertEqual(markers[".codex/hooks.json"],
                         "metadata.managed_by_jig")


class ManagedFileMetadataTests(unittest.TestCase):
    """Slice 033-04 — scaffolded managed files are tracked compactly in
    scaffold.json so future update tooling can detect untouched vs edited
    generated output without noisy inline metadata."""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="jig-033-04-managed-")
        self.target = Path(self.tmpdir) / "demo-project"
        self.target.mkdir()
    ...
    def _managed_by_path(self) -> dict:
        files = self._manifest()["managed_files"]["files"]
    ...
    def test_manifest_records_managed_file_source_hash_and_renderer(self):
        r = run_scaffold_with_args(self.target)
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        managed = self._manifest()["managed_files"]
        self.assertEqual(managed["schema_version"], 1)
        self.assertEqual(managed["policy"]["default"], "manifest-only")

        by_path = self._managed_by_path()
        for rel, source in {
            "AGENTS.md": "templates/AGENTS.md.template",
            "CLAUDE.md": "templates/CLAUDE.md.template",
            ".claude/skills/jig-scaffold-init/SKILL.md":
                "skills/scaffold-init/SKILL.md",
            ".claude/agents/jig-reviewer.md": "agents/reviewer.md",
            ".claude/hooks/scripts/jig-context-check.sh":
                "hooks/scripts/jig-context-check.sh",
            ".claude/hooks/scripts/lib/context_fill.py":
                "hooks/scripts/lib/context_fill.py",
            ".claude/settings.json": "hooks/hooks.json",
        }.items():
            self.assertIn(rel, by_path)
            entry = by_path[rel]
            self.assertEqual(entry["source"], source)
            self.assertEqual(entry["jig_version"], scaffold_mod.JIG_VERSION)
            self.assertEqual(entry["host_renderer"], "claude")
            self.assertEqual(entry["metadata"], "scaffold.json")
            self.assertEqual(
                entry["content_hash"],
                self._sha256(self.target / rel),
            )

    def test_large_prompt_files_do_not_gain_inline_hash_metadata(self):
        r = run_scaffold_with_args(self.target)
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")

        for rel in ("AGENTS.md", "CLAUDE.md",
                    ".claude/skills/jig-spec-workflow/SKILL.md",
                    ".claude/agents/jig-implementer.md"):
            text = (self.target / rel).read_text()
            self.assertNotIn("sha256:", text, f"inline hash leaked into {rel}")
            self.assertNotIn(
                "managed_files", text,
                f"manifest metadata leaked into {rel}",
            )

    def test_plugin_only_manifest_has_no_runtime_or_codex_entries(self):
        r = run_scaffold_with_args(self.target, "--plugin-only")
        self.assertEqual(r.returncode, 0, f"stderr: {r.stderr}")
        ...
        self.assertFalse(
            any(path.startswith(".claude/") for path in by_path),
            f"plugin-only manifest should not track runtime files: {by_path}",
        )
        self.assertFalse(any(path.startswith(".codex/") for path in by_path))
        self.assertFalse(any(path.startswith(".agents/") for path in by_path))
        self.assertFalse(
            any(path.startswith(".codex-plugin/") for path in by_path)
        )

    def test_manifest_does_not_claim_preexisting_runtime_files(self):
        user_skill = self.target / ".claude" / "skills" / "user-skill"
        user_skill.mkdir(parents=True)
        (user_skill / "SKILL.md").write_text(
            "---\nname: user-skill\n---\n# User skill\n"
        )
        user_agent = self.target / ".claude" / "agents" / "jig-personal.md"
        user_agent.parent.mkdir(parents=True)
        user_agent.write_text("# Personal agent\n")
        user_scripts = self.target / ".claude" / "hooks" / "scripts"
        (user_scripts / "lib").mkdir(parents=True)
        (user_scripts / "user-owned.sh").write_text("#!/usr/bin/env bash\n")
        (user_scripts / "lib" / "user_owned.py").write_text("# user helper\n")
        ...
        for rel in (
            ...
            ".claude/agents/jig-personal.md",
            ".claude/hooks/scripts/user-owned.sh",
            ".claude/hooks/scripts/lib/user_owned.py",
        ):
            self.assertTrue((self.target / rel).is_file())
            self.assertNotIn(rel, by_path)

    def test_force_rerun_over_untouched_managed_files_is_idempotent(self):
        first = run_scaffold_with_args(self.target)
        self.assertEqual(first.returncode, 0, f"stderr: {first.stderr}")
        first_manifest = self._manifest()
        first_agents = (self.target / "AGENTS.md").read_text()

        second = run_scaffold_with_args(self.target, "--force")
        self.assertEqual(second.returncode, 0, f"stderr: {second.stderr}")
        second_manifest = self._manifest()

        self.assertEqual(first_manifest["timestamp"], second_manifest["timestamp"])
        self.assertEqual(
            first_manifest["managed_files"],
            second_manifest["managed_files"],
        )
        self.assertEqual(first_agents, (self.target / "AGENTS.md").read_text())

    def test_force_rerun_refuses_user_edited_managed_file(self):
        first = run_scaffold_with_args(self.target)
        self.assertEqual(first.returncode, 0, f"stderr: {first.stderr}")
        ...
        self.assertIn("managed file changed", second.stderr.lower())
        self.assertIn("AGENTS.md", second.stderr)
        self.assertIn("User-owned edit.", agents.read_text())

    def test_metadata_policy_and_update_path_are_documented(self):
        architecture = (REPO_ROOT / "docs" / "architecture.md").read_text()
        self.assertIn("manifest-only", architecture)
        self.assertIn("inline", architecture)
        self.assertIn("scaffold.json", architecture)

        refinement = (REPO_ROOT / "docs" / "refinement-todo.md").read_text()
        self.assertIn("jig update", refinement)
        self.assertIn("033-04", refinement)
...
class ScaffoldCompletionMarkerTests(unittest.TestCase):
    ...
    def test_watermark_constant_is_present_in_primer_templates(self):
        """Slice 032-02 reconciliation pin — `_is_jig_partial_state` reads
        primer files for `_JIG_CLAUDE_MD_WATERMARK`. If the watermark string
        ever drifts out of the generated primer templates, the recovery
        path for AC #2 silently breaks (the gate returns False and
        `_looks_already_spec_driven` blocks the re-run). Pin the coupling
        with a substring check on both template files."""
